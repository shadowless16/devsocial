...
export async function GET(req: NextRequest) {
  try {
    ...
    const isAnonymousParam = searchParams.get("isAnonymous");

    // Build match filter
    const matchFilter: any = { author: { $ne: null } };
    if (authorId) {
      matchFilter.author = new mongoose.Types.ObjectId(authorId);
    }
    if (isAnonymousParam !== null) {
      matchFilter.isAnonymous = isAnonymousParam === 'true';
    }
    ...
    const totalPosts = await Post.countDocuments(matchFilter);
...
export async function POST(req: NextRequest) {
  try {
    ...
    // NOW fetch the populated post to include new XP/Level in the response
    const populatedPost = await Post.findById(newPost._id)
      .populate("author", "username firstName lastName avatar level role gender displayName points") // Added points here too
      .lean();
    ...
    const transformedPost = {
      ...
      author: postAuthor ? {
        ...
        points: postAuthor.points || 0, // Ensure points is included
      } : null,
