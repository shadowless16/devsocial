...
export class GamificationService {
  static async awardXP(
    ...
    try {
      ...
      if (!userStats) {
        // Find user to get current data or initialize new stats
  ...
  static async getLeaderboard(type: "all-time" | "weekly" | "monthly" = "all-time", limit = 50) {
    ...
    const sortField = (type === "all-time" ? "totalXP" : type === "weekly" ? "weeklyXP" : "monthlyXP") as keyof IUserStats

    // Use lean() to get plain JavaScript objects and avoid Document type issues
    const leaderboard = await UserStats.find()
      .sort({ [sortField]: -1 })
      .limit(limit)
      .populate("user", "username avatar")
      .lean() as unknown as (IUserStats & { user: { username: string; avatar: string } })[]

    return leaderboard.map((entry, index) => ({
      rank: index + 1,
      username: entry.user?.username || 'Unknown', // Safe navigation
      avatar: entry.user?.avatar || '',
      xp: entry[sortField] || 0, // Safe access
  ...
  static async getUserProgress(userId: string) {
    ...
    const userStats = await UserStats.findOne({ userId })
      .populate("user", "username avatar")
      .lean() as unknown as (IUserStats & { user: { username: string; avatar: string } }) | null
